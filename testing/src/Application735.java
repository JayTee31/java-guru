...
public class Application735 {
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            while (true) {
                ...
                if (isWordContainsGuess(wordToGuessInChars, guess)) {
                    if (isAsteriskAlreadyContainsGuess(asteriskArray, guess)) {
                        System.out.printf("%c is already in the word\n", guess);
                        continue;
                    } else {
                        asteriskArray = showGuessInWord(asteriskArray, wordToGuessInChars, guess);
                    }
    ...
    public static boolean isWordContainsGuess(final char[] word, final char guess) {
        return Arrays.toString(word).contains(String.valueOf(guess));
    }

    public static boolean isAsteriskAlreadyContainsGuess(final char[] asteriskArray, final char guess) {
        return Arrays.toString(asteriskArray).contains(String.valueOf(guess));
    }

    public static char[] showGuessInWord(char[] asteriskArray, final char[] word, final char guess) {
